...
#[test]
fn parse_package_declaration_with_leading_separator() -> Result<(), Box<dyn std::error::Error>> {
    let mut parser = Parser::new("package ::My::App;");
    let ast = must(parser.parse());
...
#[test]
fn parse_package_declaration_with_leading_and_trailing_separator_block(
) -> Result<(), Box<dyn std::error::Error>> {
    let mut parser = Parser::new("package ::My::App:: { my $x = 1; }");
    let ast = must(parser.parse());
